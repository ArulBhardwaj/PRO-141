...
def recommend_movies():
  global liked_movies
  col_names=['original_title','poster_link','release_date','runtime','weighted_runtime']
  all_recommended=pd.DataFrame(columns=col_names)

  for liked_movie in liked_movies:
    output = get_recommendations(liked_movie["original_title"])
    all_recommend=all_recommended.append(output)

  all_recommended_drop_duplicates(subset=["original_title"].inplace=True)

  recommended_movie_data=[]

  for index,row in all_recommnded.iterrows():
    _p={
      "original_title":row["original_title"],
      "poster_link":row['poster_link'],
      "release_date":row['release_date'] or "N/A",
